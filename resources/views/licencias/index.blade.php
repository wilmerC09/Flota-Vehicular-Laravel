        ...
        <section class="content-header" style="padding: 1.5rem 1rem; background: white; border-bottom: 1px solid #e5e7eb;">
            <div class="container-fluid">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h1 style="font-size: 24px; font-weight: 700; color: #1f2937; margin: 0;">Licencias</h1>
                        ...
                        <table class="table table-hover mb-0">
                            <thead style="background: #f9fafb;">
                                <tr>
                                    ...
                                    <th style="padding: 12px 16px; font-weight: 600; font-size: 13px; color: #6b7280;"><i
                                            class="fas fa-user text-muted mr-1"></i>Conductor</th>
                                    <th style="padding: 12px 16px; font-weight: 600; font-size: 13px; color: #6b7280;"><i
                                            class="fas fa-id-card text-muted mr-1"></i>Número</th>
                                    <th style="padding: 12px 16px; font-weight: 600; font-size: 13px; color: #6b7280;"><i
                                            class="fas fa-certificate text-muted mr-1"></i>Tipo</th>
                                    <th style="padding: 12px 16px; font-weight: 600; font-size: 13px; color: #6b7280;"><i
                                            class="fas fa-calendar text-muted mr-1"></i>Fecha Expedición</th>
                                    <th style="padding: 12px 16px; font-weight: 600; font-size: 13px; color: #6b7280;"><i
                                            class="fas fa-calendar-times text-muted mr-1"></i>Fecha Vencimiento</th>
                                    <th style="padding: 12px 16px; font-weight: 600; font-size: 13px; color: #6b7280;"><i
                                            class="fas fa-building text-muted mr-1"></i>Entidad Emisora</th>
                                    <th style="padding: 12px 16px; font-weight: 600; font-size: 13px; color: #6b7280;"><i
                            ...
                            <tbody>
                                @forelse($licencias as $licencia)
                                    <tr style="border-bottom: 1px solid #f3f4f6;">
                                        ...
                                        <td style="padding: 12px 16px; color: #6b7280; font-weight: 600;">{{ $licencia->id }}
                                        </td>
                                        <td style="padding: 12px 16px; font-weight: 600; color: #1f2937;">
                                            {{ $licencia->conductor->nombre ?? 'N/A' }}
                                            {{ $licencia->conductor->apellido ?? '' }}
                                        </td>
                                        <td style="padding: 12px 16px; color: #4b5563;">{{ $licencia->numero_licencia }}</td>
                                        ...
                                        <td style="padding: 12px 16px; color: #4b5563;">
                                            <i class="fas fa-calendar-times mr-1" style="color: #ef4444;"></i>
                                            {{ \Carbon\Carbon::parse($licencia->fecha_vencimiento)->format('d/m/Y') }}
                                        </td>
                                        <td style="padding: 12px 16px; color: #4b5563;">{{ $licencia->entidad_emisora }}</td>
                                        <td style="padding: 12px 16px; color: #4b5563;">{{ $licencia->registrado_por }}</td>
                                        <td style="padding: 12px 16px; text-align: center;">
                                            <span class="status-badge" data-id="{{ $licencia->id }}" data-model="licencias"
                                                data-status="{{ $licencia->estado }}"
                                                style="background: {{ $licencia->estado ? '#d1fae5' : '#fee2e2' }}; color: {{ $licencia->estado ? '#047857' : '#dc2626' }}; padding: 6px 14px; border-radius: 12px; font-size: 12px; font-weight: 600; cursor: pointer;"
                                                onclick="toggleStatus(this)">
                                                {{ $licencia->estado ? 'Activo' : 'Inactivo' }}
                                            </span>
                                        </td>
                                        <td style="padding: 12px 16px; text-align: center;">
                                            <button onclick="window.location='{{ route('licencias.edit', $licencia->id) }}'"
                                                style="background: transparent; border: none; color: #6b7280; padding: 6px 8px;"><i
                                                    class="fas fa-edit"></i></button>
                                            <button onclick="confirmDelete({{ $licencia->id }})"
                                                style="background: transparent; border: none; color: #ef4444; padding: 6px 8px;"><i
                                                    class="fas fa-trash"></i></button>
                                            <form id="delete-form-{{ $licencia->id }}"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="11" class="text-center" style="padding: 40px;"><i class="fas fa-inbox"
                                                style="font-size: 48px; color: #9ca3af; display: block; margin-bottom: 16px;"></i>
                                            <p style="color: #9ca3af;">No hay licencias registradas</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
