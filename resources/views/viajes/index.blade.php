        ...
        <section class="content-header" style="padding: 1.5rem 1rem; background: white; border-bottom: 1px solid #e5e7eb;">
            <div class="container-fluid">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h1 style="font-size: 24px; font-weight: 700; color: #1f2937; margin: 0;">Viajes</h1>
                        <nav style="font-size: 14px; color: #6b7280; margin-top: 4px;"><a href="{{ route('home') }}"
                                ...
                                style="margin: 0 8px;">/</span><span>Viajes</span></nav>
                    </div><a href="{{ route('viajes.create') }}"
                        style="background: #1f2937; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 500; font-size: 14px; display: inline-flex; align-items-center; gap: 8px;"><i
                            class="fas fa-plus"></i> Nuevo Viaje</a>
                </div>
            </div>
        </section>
                        ...
                        <table class="table table-hover mb-0">
                            <thead style="background: #f9fafb;">
                                <tr>
                                    ...
                                    <th style="padding: 12px 16px; font-weight: 600; font-size: 13px; color: #6b7280;"><i
                                            class="fas fa-car text-muted mr-1"></i>Vehículo</th>
                                    <th style="padding: 12px 16px; font-weight: 600; font-size: 13px; color: #6b7280;"><i
                                            class="fas fa-route text-muted mr-1"></i>Ruta</th>
                                    <th style="padding: 12px 16px; font-weight: 600; font-size: 13px; color: #6b7280;"><i
                                            class="fas fa-tachometer-alt text-muted mr-1"></i>Recorrido (km)</th>
                                    <th style="padding: 12px 16px; font-weight: 600; font-size: 13px; color: #6b7280;"><i
                                            class="fas fa-dollar-sign text-muted mr-1"></i>Costo Total</th>
                                    <th
                            ...
                            <tbody>
                                @forelse($viajes as $viaje)
                                    <tr style="border-bottom: 1px solid #f3f4f6;">
                                        ...
                                        <td style="padding: 12px 16px; color: #6b7280; font-weight: 600;">{{ $viaje->id }}</td>
                                        <td style="padding: 12px 16px; font-weight: 600; color: #1f2937;">
                                            {{ $viaje->conductor->nombre ?? 'N/A' }} {{ $viaje->conductor->apellido ?? '' }}
                                        </td>
                                        <td style="padding: 12px 16px; color: #4b5563;">{{ $viaje->vehiculo->placa ?? 'N/A' }}
                                        </td>
                                        <td style="padding: 12px 16px; color: #4b5563;">{{ $viaje->ruta->nombre_ruta ?? 'N/A' }}
                                        </td>
                                        <td style="padding: 12px 16px; color: #4b5563;">
                                            {{ number_format($viaje->recorrido, 2) }} km
                                        </td>
                                        <td style="padding: 12px 16px; color: #10b981; font-weight: 600;">
                                            ${{ number_format($viaje->costo_total, 2) }}</td>
                                        <td style="padding: 12px 16px; text-align: center;"><span class="status-badge"
                                                data-id="{{ $viaje->id }}" data-model="viajes"
                                                data-status="{{ $viaje->estado }}"
                                                style="background: {{ $viaje->estado ? '#d1fae5' : '#fee2e2' }}; color: {{ $viaje->estado ? '#047857' : '#dc2626' }}; padding: 6px 14px; border-radius: 12px; font-size: 12px; font-weight: 600; cursor: pointer;"
                                                onclick="toggleStatus(this)">{{ $viaje->estado ? 'Activo' : 'Inactivo' }}</span>
                                        </td>
                                        <td style="padding: 12px 16px; text-align: center;"><button
                                                onclick="window.location='{{ route('viajes.edit', $viaje->id) }}'"
                                                style="background: transparent; border: none; color: #6b7280; padding: 6px 8px;"><i
                                                    class="fas fa-edit"></i></button><button
                                                onclick="confirmDelete({{ $viaje->id }})"
                                                style="background: transparent; border: none; color: #ef4444; padding: 6px 8px;"><i
                                                    class="fas fa-trash"></i></button>
                                            <form id="delete-form-{{ $viaje->id }}"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="9" class="text-center" style="padding: 40px;"><i class="fas fa-inbox"
                                            ...
                                            <p style="color: #9ca3af;">No hay viajes registrados</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
